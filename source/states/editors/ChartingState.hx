...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function onChartLoaded()
	{
		...
		keyStepper.value = PlayState.SONG.mania; 
	...
	function createGrids()
	{
		...
		GRID_COLUMNS_PER_PLAYER = PlayState.SONG.mania + 1 ?? 4;
	...
	function addSongTab()
	{
		...
		keyStepper = new PsychUINumericStepper(objX + 140, objY + 40, 1, 1, 0, 7, 0);
		keyStepper.onValueChange = function() PlayState.SONG.mania = Math.floor(keyStepper.value);
		...
		tab_group.add(new FlxText(scrollSpeedStepper.x, scrollSpeedStepper.y - 15, 80, 'Keys:'));
		...
		tab_group.add(playerDropDown);
			...
			var keyStepper:PsychUINumericStepper;
